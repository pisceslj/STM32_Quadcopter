...
float psp=280,psd,psi,pcp,pcd;
float rsp,rsd,rsi,rcp,rcd;
float ysp,ysd,ysi,ycp,ycd;
...
//MOTOR
	...
	{
		//x 模式
		moto3 = Rc_Get.THROTTLE - 200 - PID_ROL.core_out - PID_PIT.core_out + PID_YAW.core_out;
		moto4 = Rc_Get.THROTTLE - 200 + PID_ROL.core_out - PID_PIT.core_out - PID_YAW.core_out;
		moto1 = Rc_Get.THROTTLE - 200 + PID_ROL.core_out + PID_PIT.core_out + PID_YAW.core_out;
		moto2 = Rc_Get.THROTTLE - 200 - PID_ROL.core_out + PID_PIT.core_out - PID_YAW.core_out;
		//+ 模式
...
/*********************************/
	...
	PID_ROL.shell_D = 0;//10                 
	PID_ROL.shell_I = 0;//0.08
	
/*********************************/
	PID_YAW.shell_P = 1.5;//10;//30
	PID_YAW.shell_D = 0;//10                 
	PID_YAW.shell_I = 0;//0.08;//0.08
...
//float Pitch_core_kd=0.008;////0.007;//0.07;
  ...
  PID_YAW.core_P = 0.012;////0.007;//06;//0.07;
}
